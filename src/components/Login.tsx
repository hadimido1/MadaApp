...
import { User, Trash2, ExternalLink, HelpCircle, Copy, Check, X, Shield, Chrome } from 'lucide-react';
          ...
          >
            ...
            <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 light-mode-bg light-mode-border">
              ...
              <p className="text-xs text-gray-400 leading-relaxed mb-3">
                {lang === 'ar' 
                  ? 'جوجل تمنع تسجيل الدخول داخل الـ WebView الافتراضي للأمان. لكي تجعل تسجيل جوجل يعمل مباشرة داخل تطبيقك الـ APK دون مغادرته أبداً، اتبع الخطوات البسيطة التالية:'
                  : 'Google blocks default WebView login for security. To make Google Login work seamlessly inside your APK directly, follow these simple steps:'}
              </p>

              <ol className="text-xs text-gray-400 list-decimal list-inside space-y-1.5 mb-4 text-right" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                <li>
                  {lang === 'ar' ? 'افتح مشروعك في ' : 'Open your project in '}
                  <strong className="text-white light-mode-text">Kodular</strong>
                </li>
                <li>
            ...
            <p className="text-[10px] text-gray-500 mt-6 text-center leading-relaxed">
              {lang === 'ar' 
                ? 'ملاحظة: هذا القيد مفروض من شركة Google لحمايتك، وتغيير الـ UserAgent هو الطريقة الرسمية المعتمدة للمطورين لتخطي هذا القيد بأمان.'
                : 'Note: This restriction is enforced by Google for your security. Changing the UserAgent is the standard developer way to safely authenticate.'}
            </p>
          </div>
        </div>
      )}
